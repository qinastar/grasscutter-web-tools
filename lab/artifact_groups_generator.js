/***
 * 数据来源: https://github.com/wmn1525/grasscutterTools/blob/master/src/pages/start/components/json/holyrelicname.json，感谢整理！
 * 数据已进行重新分类整理，生成更佳精确的表
 */
...
const classifyArtifact = (children) => {
  ...
  children.forEach(item => {
    ...
    const typeName = typeMapping[(item.value + '').substring(3, 4)]
    const attrsCount = item.value % 10;
    const starGroup = starGroups[starNum] || {
      flower: [],
      feather: [],
      sand: [],
      cup: [],
    ...
    starGroup[typeName].push({
      id: item.value,     // id
      name: artName,      // 名称
      type: typeName,     // 圣遗物类型
      attrsCount,         // 圣遗物初始词条数（似乎没有啥用）
    })
    starGroup[typeName].sort((a, b) => a.attrsCount > b.attrsCount ? -1 : 1);  // 倒序排列
...
fs.writeFile('./artifact_groups_map.json', JSON.stringify(formattedData, null,2), () => {
  console.log('写入完成')
});
